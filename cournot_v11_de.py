...
def objective_de(xn,x_allmn,n,cn,b, ini_assets):
    #n's profit for strategy xn given full array x
    #...
    #same as normal, but normalise the input array.
    ...
    #print xn
    #print xn[0]
    xn[0] = normalize(xn[0],axis=1,norm='l1')
    #print xn
    #raw_input('')
    x = np.insert(x_allmn,n,xn,0)
    #print 'xn',xn
...
def reaction(x_allmn,cn,b,n,n_cos,n_years, ini_assets):
    ...
    xn = optimize.differential_evolution(objective_de, tpl_bnds[0], args = minimizer_kwargs['args'],popsize=20,polish=False)#, disp=True,callback=call_de) 
    ...
    return xn.x #normalize(xn.x, axis=1, norm='l1')

def call (x,f,accept):
	print x.reshape(5,5)
	print f

def call_de (x,convergence):
	print np.round_(normalize(x.reshape(5,5),axis=1,norm='l1'),decimals=2,out=None)
